    ...
    if download:
        ...
        for i, pmid in enumerate(pmids):
            ...
            matching_file = filter_filepaths(
                fpath=args.files_downloaded,
                identifiers=['biocjson', pmid]
            )
            ...
            if len(matching_file)==1: 
                ...
                biocjson_fpath = matching_file[0]
                logger.info(f"Warning: More than one matching biocjson file found for pmid {pmid}. {matching_file}. Reading in {biocjson_fpath} only.")
            else:
